...
<body>
	...
	<form name="form1" method="post" action="/board/list.do">
		<select name="search_option">
			<option value="all"
				<c:out value="${map.search_option == 'all' ? 'selected' : ''}" />>이름
				+ 내용 + 제목</option>
			<option value="name"
	...
	<table border="1" width="600px">
		...
		<c:forEach var="row" items="${map.list}">
			<tr>
				<td>${row.idx}</td>
				<td><a
					href="/board/detail.do?idx=${row.idx}&cur_page=${map.page_info.curPage}&search_option=${map.search_option}&keyword=${map.keyword}">${row.title}</a>
					<c:if test="${row.cnt > 0}">
